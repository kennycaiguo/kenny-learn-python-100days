# """
# 要使用redis,首先需要下载redis:https://github.com/tporadowski/redis/releases
# 然后安装软件,然后把redis-server安装为服务
# 使用这个目录测试安装是否成功
# C:\Users\kenny>redis-cli.exe -h 127.0.0.1 -p 6379
# 127.0.0.1:6379> ping
# PONG
# 127.0.0.1:6379>
# 这样子,就说明redis安装成功
# 然后我们需要安装python redis支持: pip install redis
...
def main():
    ...
    #                         password='123123')
    cli = redis.Redis(host="127.0.0.1", port=6379, password=None, decode_responses=True) # 无密码连接
    data = base64.b64decode(cli.get('guido'))
    # with open('guido2.jpg', 'wb') as file_stream:
    #     file_stream.write(data)
    # with open('guido2.jpg', 'rb') as file_stream:
    #     result = base64.b64encode(file_stream.read())
    # cli.set('guido2', result)
    # 先设置
    # persons = [
