...
### 添加注册模板文件以及注册路由以及注册视图函数

#### 先创建注册模板文件register.html
...
#### 然后我们在vote/urls.py里面创建一个路由映射register/,把它和polls/views.py里面的register函数对应起来

![image-20260707102807899](./49.Cookie和Session.assets/image-20260707102807899.png)

#### 然后我们在polls/views.py里面创建这个register函数
...
![image-20260707113015145](./49.Cookie和Session.assets/image-20260707113015145.png)

![image-20260707113115689](./49.Cookie和Session.assets/image-20260707113115689.png)

![image-20260707113214137](./49.Cookie和Session.assets/image-20260707113214137.png)

![image-20260707113240771](./49.Cookie和Session.assets/image-20260707113240771.png)

![image-20260707113309832](./49.Cookie和Session.assets/image-20260707113309832.png)
...
编写生成验证码图片的类`Captcha`。放在utils模块里面？
...
class Captcha:
    ...
    def _text(self, captcha_text, fonts, font_sizes=None, drawings=None, squeeze_factor=0.75, color=None):
        ...
        for c in captcha_text:
            ...
            # c_width, c_height = draw.textsize(c, font=font) # textsize方法已经过时并且被移除了，用textbox来代替
...
上面的代码中，我们设定了登录成功后会在session中保存用户的编号（`userid`）和用户名（`username`），页面会重定向到首页。接下来我们可以稍微对首页的代码进行调整，在页面的右上角显示出登录用户的用户名。我们将这段代码单独写成了一个名为header.html的HTML文件，首页（subjects.html）中可以通过在`<body>`标签中添加`{% include 'header.html' %}`来包含这个页面，代码如下所示。
...
```Python
def good_or_bad(request: HttpRequest) -> HttpResponse:
