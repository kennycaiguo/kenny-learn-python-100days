...
配置DRF，在项目默认应用程序里面的settings.py里面添加到INSTALLED_APPS里面
...
@api_view(('GET', ))
def show_subjects_api2(request: HttpRequest) -> HttpResponse:
...
## 然后我们在urls.py里面的'api/subjects/'用show_subjects_api2代替show_subjects_api

对比上一个章节的使用`bpmapper`实现模型序列化的代码，使用DRF的代码更加简单明了，而且DRF本身自带了一套页面，可以方便我们查看我们使用DRF定制的数据接口，如下图所示。

![image-20260706162359691](./54.RESTful架构和DRF入门.assets/image-20260706162359691.png)
...
```Python
@api_view(('GET', ))
def get_teachers_api(request: HttpRequest) -> HttpResponse:
    try:
...
urlpatterns = [
    ...
    path('api/teachers/query', get_teachers_api),
