...
![image-20260707081924863](./56.使用缓存.assets/image-20260707081924863.png)

### 首先，你需要在你的redis服务里面配置密码

为Redis设置密码最简单的方法是使用命令行直接设置，或通过修改配置文件使其永久生效。命令行修改立即可用，而修改文件可防止重启后失效。 [[1](https://blog.csdn.net/qq_25667815/article/details/107709967), [2](https://redis.com.cn/redis-security.html), [3](https://juejin.cn/post/7336092523771002919)]

方式一：命令行设置（临时生效）

此方法无需重启服务即可生效，但重启Redis后密码会失效。 [[1](https://blog.csdn.net/lovemysea/article/details/70766527), [2](https://juejin.cn/post/7336092523771002919)]

1. 使用客户端连接Redis：
   `redis-cli`
2. 设置密码（例如将密码设为 `mypassword`）：
   `config set requirepass mypassword`
3. 验证密码：
   `auth mypassword`
   （系统返回 `OK` 即表示成功） [[1](https://blog.csdn.net/lovemysea/article/details/70766527), [2](https://juejin.cn/post/7336092523771002919)]

方式二：修改配置文件（永久生效）

若希望重启服务器后密码依然有效，请修改配置文件。 [[1](https://blog.csdn.net/qq_25667815/article/details/107709967)]

1. 找到并打开Redis配置文件（通常为 `redis.conf`）。
2. 找到 `# requirepass foobared` 这一行。
3. 取消注释（删除最前面的 `#`），并将 `foobared` 替换为你的新密码：
   `requirepass mypassword`
4. 保存文件并重启Redis服务器即可。 [[1](https://cloud.tencent.com/developer/article/1886285), [2](https://www.cnblogs.com/blogzys/p/18755231), [3](https://www.cnblogs.com/suanshun/p/7699084.html)]

建议参考 **[\**\*\*![img](data:image/png;base64,)\*\**\*⁠Redis 密码设置和查看密码](https://www.cnblogs.com/suanshun/p/7699084.html)** 了解更多细节，或查看 **[\**\*\*![img](data:image/png;base64,)\*\**\*⁠redis设置永久密码](https://blog.csdn.net/qq_25667815/article/details/107709967)**

### 需要密码的redis服务如何在命令行登录

在命令行登录带密码的Redis服务有两种主要方法：可以在连接时直接输入密码，也可以先连接再输入密码验证。 [[1](https://comate.baidu.com/zh/page/bcrbhdpetny)]

方法一：连接时直接输入密码

在命令行输入以下命令。将 `your_password` 替换为你的真实密码。 [[1](https://comate.baidu.com/zh/page/bcrbhdpetny)]
...
![image-20260707083059880](./56.使用缓存.assets/image-20260707083059880.png)。

*(注意：此方法会在屏幕上明文显示密码，适合个人测试环境使用。)* [[1](https://comate.baidu.com/zh/page/bcrbhdpetny)]
...
如果你的Redis部署在其他服务器上，需要加上 `-h`（主机地址）和 `-p`（端口号）参数： [[1](https://www.cnblogs.com/ShineLeBlog/p/17638005.html)]
...
修改我们settings.py中Django配置文件中关于缓存的配置。
...
### 我们可以在urls.py里面把api/subjects/路径的函数改为show_subjects_api_cache，也是可以正常工作的

![image-20260706171101423](./56.使用缓存.assets/image-20260706171101423.png)

![image-20260706171126334](./56.使用缓存.assets/image-20260706171126334.png)
...
1. Cache Aside Pattern   # 非常繁琐
2. Read/Write Through Pattern # 这种是比较好
3. Write Behind Caching Pattern # 比较复杂
