...
如果想调试你的Django项目，你一定不能错过名为Django-Debug-Toolbar的神器，它是项目开发阶段辅助调试和优化的必备工具，只要配置了它，就可以很方便的查看到如下表所示的项目运行信息，这些信息对调试项目和优化Web应用性能都是至关重要的。
...
```SQL
SELECT `teacher`.`no`, `teacher`.`name`, `teacher`.`intro`, `teacher`.`photo`, `teacher`.`gcount`, `teacher`.`bcount`, `teacher`.`sno` FROM `teacher`; args=()
SELECT `subject`.`no`, `subject`.`name`, `subject`.`intro`, `subject`.`create_date`, `subject`.`is_hot` FROM `subject` WHERE `subject`.`no` = 101; args=(101,)
SELECT `subject`.`no`, `subject`.`name`, `subject`.`intro`, `subject`.`create_date`, `subject`.`is_hot` FROM `subject` WHERE `subject`.`no` = 101; args=(101,)
SELECT `subject`.`no`, `subject`.`name`, `subject`.`intro`, `subject`.`create_date`, `subject`.`is_hot` FROM `subject` WHERE `subject`.`no` = 101; args=(101,)
SELECT `subject`.`no`, `subject`.`name`, `subject`.`intro`, `subject`.`create_date`, `subject`.`is_hot` FROM `subject` WHERE `subject`.`no` = 101; args=(101,)
SELECT `subject`.`no`, `subject`.`name`, `subject`.`intro`, subject`.`create_date`, `tb_subject`.`is_hot` FROM `subject` WHERE `subject`.`no` = 103; args=(103,)
SELECT `subject`.`no`, `subject`.`name`, `subject`.`intro`, `subject`.`create_date`, `subject`.`is_hot` FROM `subject` WHERE `subject`.`no` = 103; args=(103,)
```
...
```Python
queryset = Teacher.objects.all().only('name', 'gcount', 'bcount')
```
...
```Python
queryset = Teacher.objects.values('subject').annotate(good=Avg('gcount'), bad=Avg('bcount'))
```
...
```Python
queryset = Teacher.objects.values('subject__name').annotate(good=Avg('gcount'), bad=Avg('bcount'))
```
