...
### 说明：原来这里的表格都是以tb_开头，我觉得很麻烦，所以把这个前缀去掉直接使用表名称入subject，teacher等等
...
2. 在MySQL中创建保存学科和老师信息的二维表（保存用户信息的表稍后处理）。
   ...
   (
       ...
       `subject` integer not null comment '所属学科',
       primary key (`no`),
       foreign key (`subject`) references `subject` (`no`)
   );
   ```
...
5. Django框架提供了ORM来解决数据持久化问题，ORM翻译成中文叫“对象关系映射”。因为Python是面向对象的编程语言，我们在Python程序中使用对象模型来保存数据，而关系型数据库使用关系模型，用二维表来保存数据，这两种模型并不匹配。使用ORM是为了实现对象模型到关系模型的**双向转换**，这样就不用在Python代码中书写SQL语句和游标操作，因为这些都会由ORM自动完成。利用Django的ORM，我们可以直接将刚才创建的学科表和老师表变成Django中的模型类。，最好打开一个cmd窗口，pycharm的终端是powshell，使用它生成模型或有问题，默认cmd的编码表示utf8，生成的模型文件又会有很多乱码，所以需要打开cmd，先把编码改为utf-8，命令是chcp 65001，然后再执行python manage.py inspectdb > polls/models.py
   ...
   class Teacher(models.Model):
       ...
       # 默认生成sno我们可以把它改为subject，.ForeignKey里面的需要引号，否则报错说找不到
       class Meta:
           managed = False
           db_table = 'teacher'
   ```
...
subject3.save()
# 创建老师对象的时候需要注意，sno必须传递一个Subject类的对象
teacher1 = Teacher(name="Mary cake",sex=0,birth="1978-12-2",intro="Ok",photo="",gcount=3,bcount=5,sno=subject1)
teacher1.save()
teacher2 = Teacher(name="Linda smith",sex=0,birth="1979-12-2",intro="Good",photo="",gcount=13,bcount=2,sno=subject1)
teacher2.save()
teacher3 = Teacher(name="John smith",sex=1,birth="1977-12-22",intro="Soso",photo="",gcount=3,bcount=2,sno=subject2)
teacher3.save()
```
...
2. 创建访问`admin`应用的超级用户账号，这里需要输入用户名、邮箱和口令。为了方便学习，用户名是kenny，密码是12345
   ...
   >
   > ![image-20260701090048338](./47.深入模型.assets/image-20260701090048338.png)

3. 运行项目，在浏览器中访问`http://127.0.0.1:8000/admin`，输入刚才创建的超级用户账号和密码进行登录。

   ![image-20260701090638722](./47.深入模型.assets/image-20260701090638722.png)
    ...
    <body>
        <div id="container">
            ...
            <div id="main">
                ...
                <dl>
                    <dt>
                        <a href="/teachers/query/?sno={{ subject.no }}">{{ subject.name }}</a>
                        {% if subject.is_hot %}
...
3. 修改`vote/urls.py`文件，实现映射URL。
    ...
    urlpatterns = [
        ...
        path('', show_subjects),
        path('teachers/query/', show_teachers),
    ]
    ```
