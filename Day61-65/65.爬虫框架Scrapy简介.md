...
2. 修改`spiders`文件夹中名为`douban.py` 的文件，它是蜘蛛程序的核心，需要我们添加解析页面的代码。在这里，我们可以通过对`Response`对象的解析，获取电影的信息，代码如下所示。
   ...
   ```Shell
   PS D:\kenny-learn-python-100days\python100days-exercise-code\Day61-65-exercise\day65-scrapy\demo\demo> scrapy crawl douban
   ```

#### 可以看到此时你运行爬虫，会爬取不到数据，因为我们没有设置请求头伪造，需要在settings里面设置，我们先优化一下代码，然后在设置
...
不知大家是否注意到，通过运行爬虫获得的 JSON 文件中有`275`条数据，那是因为首页被重复爬取了。要解决这个问题，可以对上面的代码稍作调整，不在`parse`方法中解析获取新页面的 URL，而是通过`start`异步方法提前准备好待爬取页面的 URL，调整后的代码如下所示。注意：start_requests方法已经过时，使用它是爬取不到数据的。
...
3. 如果希望完成爬虫数据的持久化，可以在数据管道中处理蜘蛛程序产生的`Item`对象。例如，我们可以通过前面讲到的`openpyxl`操作 Excel 文件，将数据写入 Excel 文件中，我们在pipelines.py里面创建一个MovieItemPipeline代码如下所示。
